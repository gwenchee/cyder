...
// - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -
void ConditioningTest::SetUp() {
  src_facility_ = new Conditioning(tc_.get());
  InitParameters();
  SetUpConditioning();
}

void ConditioningTest::TearDown() {
  delete src_facility_;
}

void ConditioningTest::InitParameters(){
  in_r1 = "in_r1";
...
void ConditioningTest::TestCurrentCap(Conditioning* fac, double inv){

  EXPECT_EQ(inv, fac->current_capacity());
}

void ConditioningTest::TestReadyTime(Conditioning* fac, int t){

  EXPECT_EQ(t, fac->ready_time());
}

// - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -
TEST_F(ConditioningTest, clone) {
  Conditioning* cloned_fac =
      dynamic_cast<Conditioning*> (src_facility_->Clone());
  TestInitState(cloned_fac);
}

// - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -
TEST_F(ConditioningTest, InitialState) {
  // Test things about the initial state of the facility here
  TestInitState(src_facility_);
}

// - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -
TEST_F(ConditioningTest, CurrentCapacity){
...
// - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -
TEST_F(ConditioningTest, Print) {
  EXPECT_NO_THROW(std::string s = src_facility_->str());
  // Test Conditioning specific aspects of the print method here
}
...
TEST_F(ConditioningTest, Tick) {
  ...
  // Test Conditioning specific behaviors of the Tick function here
}

// - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -
TEST_F(ConditioningTest, Tock) {
...
// - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -
TEST_F(ConditioningTest, NoProcessTime) {
...
TEST_F(ConditioningTest, NoConvert) {
...
TEST_F(ConditioningTest, MultipleSmallBatches) {
...
TEST_F(ConditioningTest, ChangeCapacity) {
...
TEST_F(ConditioningTest, TwoBatchSameTime) {
  // Add first small batch
  double cap = throughput;
  cyclus::Composition::Ptr rec = tc_.get()->GetRecipe(in_r1);
  cyclus::Material::Ptr mat = cyclus::Material::CreateUntracked(0.2*cap, rec);
...
TEST_F(ConditioningTest,ChangeProcessTime){
...
TEST_F(ConditioningTest,DifferentRecipe){
...
TEST_F(ConditioningTest, BehaviorTest){
  // Verify Conditioning behavior
  ...
  cyclus::MockSim sim(cyclus::AgentSpec (":cycamore:Conditioning"), config, simdur);
  ...
  // return all transactions where our sconditioning facility is the sender
...
TEST_F(ConditioningTest, MultipleCommods){
  // Verify Conditioning accepting Multiple Commods
  ...
  // return all transactions where our sconditioning facility is the acceptor
  std::vector<cyclus::Cond> conds;
...
TEST_F(ConditioningTest, PositionInitialize){
  // Verify Conditioning behavior
...
TEST_F(ConditioningTest, Longitude){
  // Verify Conditioning behavior
...
// - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -
cyclus::Agent* ConditioningConstructor(cyclus::Context* ctx) {
  return new conditioning::Conditioning(ctx);
}
...
// - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -
INSTANTIATE_TEST_CASE_P(ConditioningFac, FacilityTests,
                        ::testing::Values(&ConditioningConstructor));

INSTANTIATE_TEST_CASE_P(ConditioningFac, AgentTests,
                        ::testing::Values(&ConditioningConstructor));